...
class LogPage(ttk.Frame):
    ...
    def __init__(self, parent, app_context):
        ...
        self.text = tk.Text(self, wrap="word", state="disabled", font=("Consolas", 10))
...
class _UnavailablePage(ttk.Frame):
    ...
    def __init__(self, parent, app_context, page_title: str, error_text: str):
        ...
        self.app_context = app_context
        self.columnconfigure(0, weight=1)
        ttk.Label(self, text=f"{page_title}目前無法載入", font=("Microsoft JhengHei UI", 16, "bold")).grid(row=0, column=0, sticky="w", pady=(0, 12))
        ttk.Label(self, text="請確認對應程式檔案與類別已完成，並查看紀錄頁的詳細錯誤。", wraplength=720).grid(row=1, column=0, sticky="w")
        ttk.Label(self, text=error_text, wraplength=720).grid(row=2, column=0, sticky="w", pady=(12, 0))
...
class App(tk.Tk):
    ...
    PAGE_SPECS = (
        ...
        ("opcua_server", "OPC UA Server設定", "ui.opcua_server_page", "OpcuaServerPage"),
        ("opcua_browse", "OPC UA瀏覽/掃描", "ui.opcua_browse_page", "OpcuaBrowsePage"),
    ...
    def __init__(self):
        ...
        self.config_manager = self._construct_component(ConfigManager, "ConfigManager", {"config_path": str(self.config_path)})
        self.value_bus = self._construct_component(ValueBus, "ValueBus", {})
        ...
        self.database_manager = self._construct_component(DatabaseManager, "DatabaseManager", service_dependencies)
        self.modbus_manager = self._construct_component(ModbusRtuManager, "ModbusRtuManager", service_dependencies)
        self.modbus_tcp_manager = self._construct_component(ModbusTcpManager, "ModbusTcpManager", service_dependencies)
        self.opcua_manager = self._construct_component(OpcuaMultiServerManager, "OpcuaMultiServerManager", service_dependencies)
    ...
    def _create_page(self, tab_title: str, module_name: str, class_name: str) -> ttk.Frame:
        try:
            ...
            return _UnavailablePage(self.notebook, self.app_context, tab_title, error_text)

    @staticmethod
    def _construct_component(component_class: type, component_name: str, available_values: dict[str, Any]) -> Any:
        aliases = {
        ...
        for parameter in signature.parameters.values():
            ...
            if parameter.default is inspect.Parameter.empty:
                raise TypeError(f"無法建立{component_name}：缺少必要建構參數「{parameter.name}」")
        if has_var_keyword:
        ...
        for page_key, page in tuple(self.pages.items()):
            ...
            for method_name in ("refresh", "refresh_data", "reload_data", "update_view"):
                ...
                try:
                    ...
                    self.log_func(f"頁面重新整理失敗（{page_key}.{method_name}）：{exc}", "ERROR")
                break

    def _report_callback_exception(self, exception_type: type[BaseException], exception_value: BaseException, exception_traceback) -> None:
        detail = "".join(traceback.format_exception(exception_type, exception_value, exception_traceback))
        self.log_func(detail.rstrip(), "ERROR")
        try:
            messagebox.showerror("程式錯誤", f"執行操作時發生錯誤：\n{exception_value}", parent=self)
        except tk.TclError:
            print(detail)
    ...
    def on_close(self) -> None:
        ...
        opcua_stop_method = getattr(self.opcua_manager, "shutdown", self.opcua_manager.disconnect_all)

        def cleanup_worker() -> None:
            cleanup_steps = (
                ("停止Modbus RTU輪詢", self.modbus_manager.stop_polling),
        ...
        threading.Thread(target=cleanup_worker, name="ApplicationCleanup", daemon=True).start()
