...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        set_missing_database_handler = getattr(
            self.database_manager,
            "set_missing_database_handler",
            None,
        )
        if callable(set_missing_database_handler):
            set_missing_database_handler(self._ask_to_create_missing_database)
        self.modbus_manager = self._construct_component(
            ModbusRtuManager,
            "ModbusRtuManager",
            service_dependencies,
        )
        self.modbus_tcp_manager = self._construct_component(
            ModbusTcpManager,
            "ModbusTcpManager",
            service_dependencies,
        )
    ...
    def _ask_to_create_missing_database(self, database_name: str) -> bool:
        """在Tk主執行緒詢問是否建立缺少的資料庫。"""
        prompt_text = f'偵測不到資料庫是否建立資料庫"{database_name}"'
        ...
        def ask_user() -> bool:
            ...
            try:
                ...
                self.log_func(f"顯示建立資料庫詢問視窗失敗：{exc}", "ERROR")
                raise RuntimeError("無法顯示建立資料庫詢問視窗") from exc
        ...
        try:
            ...
            self.log_func(f"排程建立資料庫詢問視窗失敗：{exc}", "ERROR")
            raise RuntimeError("無法排程建立資料庫詢問視窗") from exc

        while not completed.wait(timeout=0.1):
            if self._closing:
                return False
        if outcome["error"] is not None:
            raise RuntimeError("無法顯示建立資料庫詢問視窗") from outcome[
                "error"
            ]
        return bool(outcome["create"])
